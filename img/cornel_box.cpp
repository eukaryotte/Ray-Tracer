...
int main() {
  ...
  auto mat1 = std::make_shared<Diffuse>(Vec3(0.8)); //白色
  auto mat2 = std::make_shared<Diffuse>(Vec3(0.8, 0.2, 0.2)); //赤色
  auto mat3 = std::make_shared<Diffuse>(Vec3(0.2, 0.8, 0.2)); //緑色
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  //PPM出力
  img.ppm_output("cornell_box.ppm");
